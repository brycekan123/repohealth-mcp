...
_DEFAULT_ENTITY_SET = set(DEFAULT_ENTITIES)
DEFAULT_RANGE = "6mo"
...
Default to all entities and "6mo" if the question is general.
"""
...
def extract_plan_from_text(text: str) -> PlanResult:
    ...
    unknown_entities = set(entities) - _DEFAULT_ENTITY_SET
    if unknown_entities:
    ...
    return PlanResult(repos=list(repos), entities=list(entities), range_spec=range_spec)
